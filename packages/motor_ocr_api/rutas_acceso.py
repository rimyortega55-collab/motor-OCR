...
nada que registrar ni ninguna clave que administrar, sólo una clave de
instancia opcional que se compara contra `MOTOR_OCR_CLAVE_ACCESO`.
"""
...
def _estado(request: Request) -> dict:
    requiere = clave_configurada() is not None
    if not requiere:
...
@router.get("/acceso")
async def estado_acceso(request: Request) -> dict:
    ...
    """
    return _estado(request)


@router.post("/acceso")
async def desbloquear(pedido: Clave, respuesta: Response, request: Request) -> dict:
    ...
    if not clave_valida(pedido.clave):
        raise HTTPException(
