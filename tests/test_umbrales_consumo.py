...
def _documento(titulo: str = "doc.pdf") -> str:
    doc_id = str(uuid4())
    with session_scope() as sesion:
        sesion.add(DocumentoAlmacenado(
            id=doc_id, titulo=titulo,
...
def test_los_umbrales_son_de_la_instancia(cliente):
    """Sin cuentas, los umbrales son los de esta instancia: un cliente ve lo que guardó otro."""
    cliente.put("/api/umbrales", json={"capa3": {"parrafo": 0.88}})

    with TestClient(app) as otro:
        assert otro.get("/api/umbrales").json()["capa3"]["parrafo"] == 0.88
...
def _con_bloques() -> str:
    doc_id = _documento("mate.pdf")
    with session_scope() as sesion:
        sesion.add_all([
            BloqueAlmacenado(
...
def test_exportar_documento_inexistente_da_404(cliente):
    assert cliente.get(f"/api/documentos/{uuid4()}/export").status_code == 404
...
def test_el_acceso_tiene_tope(cliente, monkeypatch):
    """Sin tope, probar la clave de acceso a fuerza bruta sale gratis."""
    monkeypatch.setenv("MOTOR_OCR_CLAVE_ACCESO", "la-clave-correcta")
    limites.limpiar()

    with TestClient(app) as c:
        codigos = [
            c.post("/api/acceso", json={"clave": "incorrecta"}).status_code
            for _ in range(12)
        ]

    assert 429 in codigos, "se puede probar la clave sin tope"


def test_el_429_dice_cuanto_esperar(cliente, monkeypatch):
    monkeypatch.setenv("MOTOR_OCR_CLAVE_ACCESO", "la-clave-correcta")
    limites.limpiar()

    with TestClient(app) as c:
        for _ in range(12):
            r = c.post("/api/acceso", json={"clave": "incorrecta"})
