...
from ocr_engine.persistence import BloqueAlmacenado, DocumentoAlmacenado, Usuario
...
def _documento(sesion: Session, usuario: Usuario, documento_id: str) -> DocumentoAlmacenado:
    """404 y no 403 para un documento ajeno: no se confirma que exista."""
    documento = (
...
async def listar_bloques(
    ...
    documento = _documento(sesion, usuario, documento_id)
...
async def imagen_pagina(
    ...
    return FileResponse(
        ...
        headers={
            # Privado: la imagen es de un documento de un usuario, no la puede
            # cachear un proxy compartido.
