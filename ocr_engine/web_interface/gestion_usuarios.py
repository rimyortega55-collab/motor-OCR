...
from ocr_engine.persistence import Usuario, init_db, session_scope
...
def _cmd_crear(args: argparse.Namespace) -> int:
    with session_scope() as sesion:
        ...
        usuario, clave = crear_usuario(
            sesion, nombre=args.nombre, email=args.email, plan=args.plan
        )
...
def _cmd_listar(_: argparse.Namespace) -> int:
    with session_scope() as sesion:
        ...
        print(f"{'ID':38} {'NOMBRE':22} {'PLAN':10} {'CLAVE':14} ESTADO")
        for u in usuarios:
            estado = "activo" if u.activo else "desactivado"
            print(f"{u.id:38} {u.nombre[:22]:22} {u.plan:10} {u.api_key_prefijo + '...':14} {estado}")
    return 0
...
def _cmd_desactivar(args: argparse.Namespace) -> int:
    with session_scope() as sesion:
        ...
        print(f"Usuario {usuario.nombre} desactivado; su API key deja de servir.")
    return 0
