...
from ocr_engine.translation import ContextoTraduccion, bloques_a_traducir, extraer_terminos  # noqa: E402
from ocr_engine.web_interface import limites, trabajos_traduccion  # noqa: E402
...
def base_limpia():
    ...
    with session_scope() as sesion:
        for modelo in (
            ...
            BloqueAlmacenado, DocumentoAlmacenado, UmbralesUsuario, Sesion, ApiKey, Usuario,
        ):
...
def _documento(cliente, bloques: list[tuple[str, int, str]]) -> str:
    ...
    with session_scope() as sesion:
        usuario_id = sesion.query(Usuario).one().id

    documento_id = str(uuid4())
    with session_scope() as sesion:
        sesion.add(DocumentoAlmacenado(
            id=documento_id, usuario_id=usuario_id, titulo="paper.pdf",
...
def test_no_se_traduce_un_documento_a_medio_procesar(cliente):
    ...
    with session_scope() as sesion:
        sesion.add(DocumentoAlmacenado(
            id=documento_id, usuario_id=usuario_id, titulo="a medias.pdf",
...
def test_no_se_traduce_un_documento_ajeno(cliente):
    ajeno_id = str(uuid4())
    with session_scope() as sesion:
    ...
    with session_scope() as sesion:
        sesion.add(DocumentoAlmacenado(
            id=documento_id, usuario_id=ajeno_id, titulo="ajeno.pdf",
