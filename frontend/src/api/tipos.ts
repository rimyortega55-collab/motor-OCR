 ...
 * `MOTOR_OCR_CLAVE_ACCESO` sigue mandando porque nunca se rotó. `null`:
 * instancia abierta. */
export type OrigenClaveAcceso = 'panel' | 'entorno' | null

export type EstadoClaveAcceso = {
  requiere_clave: boolean
  origen: OrigenClaveAcceso
  rotada_en: string | null
}

/** Sólo la trae la respuesta de rotar, y sólo una vez: no vuelve a mostrarse
 * en ningún otro endpoint. */
export type ClaveAccesoRotada = EstadoClaveAcceso & { clave: string }
